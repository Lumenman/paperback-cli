...
Every BMP is a complete white sheet, with the data grid placed inside the margins. The last page has the same physical dimensions as the others. A multi-page backup is named `sheet_0001.bmp`, `sheet_0002.bmp`, etc.; a one-page backup uses `sheet.bmp`. BMP resolution metadata records the physical size. **Print at 100% / actual size**, with fitting, cropping and automatic enlargement disabled. Your print application must respect the BMP resolution metadata; the CLI does not control printer settings.
...
Output is saved after processing the entire input list. The summary reports recovered/missing blocks and, when page geometry is consistent, page numbers to rescan. A readable page header is required to associate its blocks with a file. Completely unreadable pages cannot contribute data.
