...
Cross-platform command-line version of Oleh Yuschuk's PaperBack. It encodes files into printable BMP pages or one PDF and restores files from scans. Files are stored as original bytes. Legacy PaperBack pages using internal compression or encryption are not supported. No AES or bzip2 dependencies are used.
...
With `-o sheet.pdf` all pages go into one PDF instead, each page exactly the paper size. The dots are a 1-bit stencil at the `--image-dpi` resolution, printed solid black; the header text stays gray. A stencil has no gray edges for a print dialog to smooth or resample, so the printer places every dot at its own resolution. On the test printer a PDF printed from Acrobat at actual size came back at 99.4-99.9% of its size with no dots lost. The same page as a BMP, printed through the Windows photo printing wizard, came back at 97% with 0.4% of its dots wrong. Print from a PDF reader at **actual size**; a browser rasterises the page itself and loses that advantage. The PDF uses RunLength compression, about 0.6 MB per A4 page at 600 dpi, so no compression library is needed. The decoder still reads scans, not PDFs.

`--dpi` is the density of code dots (40–600, default 150). `--image-dpi` is the bitmap resolution (80–2400), defaulting to three times the dot density -- set it to the printer's own resolution if you know it. On a 600 dpi laser the default of 450 makes the printer resample the sheet by 1.333 and smear every dot edge before it reaches paper; the same file printed at 600 instead read back here with no ECC repairs at all, where the 450 version lost blocks. It must be at least twice the requested dot density. Dot spacing is rounded to whole pixels; the CLI reports the resulting density. `--dotsize` is the dot width as a percentage of spacing (50–100, default 70), also rounded to pixels -- and at a small cell that rounding leaves it very few steps, so the CLI reports the cell and the dot it actually drew, in pixels and in percent. With `--image-dpi` left at its default the cell is 3 pixels, where `-s 55` and `-s 70` draw the same 2-pixel dot and `-s 85` and `-s 100` draw the same 3-pixel one: printing sheets at 55, 70 and 85 to compare them yields two settings, not three. Raising `--image-dpi` to the printer's own resolution buys both finer steps and one less resampling on the way to paper. Large page/resolution combinations exceeding 32768 pixels per side or approximately 256 MiB of sheet pixels are rejected.

`--redundancy N` adds one recovery block per N data blocks (0 or 2–10, default 5). **Smaller N provides more redundancy** and less capacity. `-r 0` lays the page out with no recovery blocks at all: labels spread over the sheet and data everywhere else, about 20% more bytes per sheet than `-r 5`. It is the same format -- a data block's address has always carried zero where a recovery block carries its group size -- so any decoder reads such a page, and simply never has a group to repair. Worth it for a sheet you can reprint or have a second copy of; for one that has to survive a coffee ring, the parity is the cheapest insurance there is, which is why it is the default. `-r 1` is refused: one recovery block per data block is a second copy of the sheet, not redundancy. `--border` enables the outer raster border; it only helps scanners with clumsy auto-cropping and does nothing for recognition, so it is off by default. `--header` prints a text band above and below the grid: file name, modification time, size and page number on top, the SHA-256 of the input file and the recommended scan resolution at the bottom. The bands take space from the grid, costing 3 to 5 % of the capacity depending on dot density, so they are off by default too. `--encode` prints the same SHA-256 to standard output whether or not the header is on, so you can record it separately. Both header lines are gray and use the same glyph size, chosen so the longer of the two fits the sheet. The footer follows the actual grid, including on short last pages, instead of sitting at the bottom of the sheet. Create archives or encrypted files using external tools before encoding. The CLI treats them as ordinary bytes and never unpacks or decrypts them.
...
The checksum each block carries for itself is 16 bits, and so is the one the page carries for the whole stored stream. Both are there to catch damage, and they do; neither is proof. A block whose Reed-Solomon correction had to be pushed to its limit rests on those 16 bits alone, and one such block in 65536 is accepted wrongly.

So the page also carries the SHA-256 of the file it holds, and the restore is checked against it with nothing kept on the side. The digest is the last 32 bytes of the stored data, in ordinary data blocks: error correction, block checksums and the recovery parity cover it exactly as they cover the file, and it is never written out, because a restore writes the original bytes and stops. It costs 32 bytes, which is one extra block on a sheet of hundreds.

Nothing in the page label announces it, and that is deliberate: a page written by this version restores unchanged under every older build, byte for byte, with no error and no warning. What gives the digest away to a decoder that knows about it is the gap between the stored size and the original size -- padding alone never reaches 32 bytes. Pages written by older versions carry no digest and are read exactly as before.

Every restore prints the SHA-256 of what it wrote. When the page carries its own, the line says the two match and the check is done. When they disagree the file is still written, both digests are reported and the exit status is 1, so a page that does not hold what it claims cannot pass silently. A partial restore says so beside its digest instead: its gaps are zeros, so it cannot match the sheet and is only good for telling two attempts at the same damaged page apart.

Losing only a block that lies past the end of the file -- padding, or the digest itself -- costs the check, not the file. The restore is complete, says so, and reports that the page's digest was not recovered. Those blocks used to count as damage, which was a false alarm: not one byte of the output comes from them.
